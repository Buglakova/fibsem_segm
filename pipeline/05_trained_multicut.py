...
def main():
    ...
    parser.add_argument("rf", type=str, help="Path of the random forest classifier")
    ...
    # Read data
    raw = read_volume(args.pipeline_n5, "input/raw_norm")
    ...
    rf_file = open(args.rf, 'rb')
    rf = pickle.load(rf_file)
    rf_file.close()
    print(rf)
    
    logging.info("Compute multicut ...")
    seg = multicut_segmentation(raw.astype(np.float32), hmap, rf, False, watershed=ws, mask=fg, n_threads=args.n_threads, beta=args.beta)

    attrs = dict(get_attrs(args.pipeline_n5, "input/raw_norm"))
    attrs["description"] = f"RF multicut, beta={args.beta}"
    write_volume(args.pipeline_n5, seg, "rf_multicut", attrs=attrs)
    
    # Plots
    plot_three_slices(seg, plots_dir / "rf_multicut.png", cmap=get_random_cmap())

...
def main():
    ...
    hmap = normalize_input(boundaries + extra + extra_mask)
    
    print(f"Raw {raw.dtype}")
    filters={"gaussianSmoothing": [1], "laplacianOfGaussian": [1], "hessianOfGaussianEigenvalues": [1]}
    seg = multicut_segmentation(raw.astype(np.float32), hmap, rf, False, watershed=ws, mask=mask, n_threads=2, beta=0.8)
    print(seg.shape)
    
    write_volume(train_multicut_path, seg, "segmentation/rf_multicut_beta08")

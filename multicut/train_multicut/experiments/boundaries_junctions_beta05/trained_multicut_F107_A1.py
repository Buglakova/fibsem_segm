...
def main():
    ...
    rf_file = open('rf_junctions.pkl', 'rb')
    ...
    hmap = normalize_input(boundaries + extra + extra_mask + junctions)
    
    print(f"Raw {raw.dtype}")
    filters={"gaussianSmoothing": [1], "laplacianOfGaussian": [1], "hessianOfGaussianEigenvalues": [1]}
    seg = multicut_segmentation(raw.astype(np.float32), hmap, rf, False, watershed=ws, mask=mask, n_threads=16, beta=0.5)
    print(seg.shape)
    
    write_volume(train_multicut_path, seg, "segmentation/rf_multicut_boundary_junctions_beta05")

...
def main():
    ...
    extra_mask = extra > 0.5
    result[extra_mask] = 2
    # result = result * (1 - extra_mask)
    # result = result + extra_mask
    # result = result.astype(np.uint64)
    write_volume(postprocess_path, result.astype(np.uint32), "segmentation/extra_renumbered")
    ...
    for seg_id in range(3, max_id + 1):
        ...
        if area > 0:
            ...
            if np.median(id_extra[id_mask]) > 0.6:
                print("Assign extracellular")
                id_multicut[id_mask] = 2
                ...
                ws_fill, _ = vigra.analysis.watershedsNew(normalize_input(id_boundaries + id_extra), seeds=id_multicut.astype(np.uint32))
                ...
                print("Renumber")
                id_multicut[id_mask] = nonzero_id
                result[rmin:rmax, cmin:cmax, zmin:zmax] = id_multicut
                print(f"Renumbered {seg_id} to {nonzero_id}")
                areas.append(area)
                nonzero_id += 1
    ...
    plt.savefig("areas_renumbered.png", dpi=300)

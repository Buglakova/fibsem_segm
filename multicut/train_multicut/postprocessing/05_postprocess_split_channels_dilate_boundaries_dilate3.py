...
def main():
    ...
    network_path = Path("/scratch/buglakova/data/cryofib/segm_fibsem/F107/F107_A1_train_network_dilated_boundaries_dilate3.n5")
    ...
    instance_boundaries = find_boundaries(multicut, mode="outer") * binary_erosion(binary_erosion(binary_erosion((fg_mask))))
    for i in range(3):
        instance_boundaries = binary_dilation(instance_boundaries)
    ...
    # boundaries = (instance_boundaries) > 0
    boundaries = boundaries.astype(np.uint16)
    extra = (multicut == 2).astype(np.uint16)
    foreground = (multicut > 2).astype(np.uint16) * (1 - boundaries)

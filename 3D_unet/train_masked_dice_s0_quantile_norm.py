...
if __name__ == "__main__":
    ...
    train_data_paths = ["/scratch/buglakova/data/cryofib/segm_fibsem/F107/F107_A1_em.n5", "/scratch/buglakova/data/cryofib/segm_fibsem/F107/F107_A1_em.n5"]
    ...
    experiment_name = "full_masked_dice_s0_16x512x512_quantile_norm_no_mask"
    ...
    z_patch_shape = 16
    patch_shape = (z_patch_shape, 512, 512)
    ...
    # ROIs with +- z patch shape // 2 frames
    dz = z_patch_shape // 2
    # train_rois = (np.s_[182:247, :, :], np.s_[262:327, :, :], np.s_[589:654, :, :], np.s_[1122:1187, :, :])
    # train_rois = [np.s_[(200 - dz):(229 + dz), :, :],
    #             np.s_[(279 - dz):(310 + dz), :, :],
    #             np.s_[(617 - dz):(626 + dz), :, :],
    #             np.s_[(1149 - dz):(1160 + dz), :, :]]
    # val_rois = np.s_[565:566, :, :]   
    train_rois = [np.s_[200:229, :, :], np.s_[279:309, :, :]]
    # val_rois = np.s_[(533 - dz):(598 + dz), :, :]
    val_rois = np.s_[200:229, :, :]
    ...
    train_loader = torch_em.default_segmentation_loader(
        ...
        batch_size, patch_shape, with_label_channels=True, raw_transform=raw_transforms, rois=train_rois,

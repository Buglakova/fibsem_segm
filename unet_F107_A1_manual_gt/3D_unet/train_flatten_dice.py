...
def get_loss(loss_name, loss_transform=None, flatten=True):
    loss_names = ["bce", "ce", "dice"]
    if isinstance(loss_name, str):
        assert loss_name in loss_names, f"{loss_name}, {loss_names}"
        if loss_name == "dice":
            loss_function = torch_em.loss.DiceLoss(flatten=flatten)
        elif loss == "ce":
...
if __name__ == "__main__":
    ...
    experiment_name = "full_masked_dice_s0_64x256x256_flatten_zlog0_noelastic_eps_nonoise"
    ...
    dz = 60
    train_rois = [np.s_[(200 - dz):(229 + dz), :, :], np.s_[(279 - dz):(310 + dz), :, :], np.s_[(617 - dz):(626 + dz), :, :], np.s_[(1149 - dz):(1160 + dz), :, :]]
    # val_rois = np.s_[565:566, :, :]
    # val_rois = np.s_[533:598, :, :]
    val_rois = np.s_[(200 - dz):(229 + dz), :, :]
    ...
    loss_function = get_loss(loss, loss_transform=MaskIgnoreLabelFlatten(-1), flatten=False)
    ...
    # trainer.fit(n_iterations, load_from_checkpoint="checkpoints/full_masked_dice_s0_64x256x256_flatten/best.pt")

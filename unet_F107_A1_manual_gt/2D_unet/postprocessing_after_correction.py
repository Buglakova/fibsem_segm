...
from skimage.morphology import remove_small_objects, binary_closing, binary_opening, binary_erosion
...
import vigra
from cryofib.data_loaders import load_F107_A1_raw, load_F107_A1_pred, load_F107_A1_multicut, load_F107_A1_corrected
...
def main():
    ...
    out_key = "segmentation/segmentation_corrected_postprocessed"

    seg = multicut.copy()
    fg_mask = get_fg_mask(raw)
    fg_mask = binary_erosion(fg_mask)
    ...
    indices, counts = fast_count_unique(seg)
    counts_nonzero = counts[counts > min_segment_size]
    indices_nonzero = indices[counts > min_segment_size]

    cur_idx = 3
    for idx, count in zip(indices_nonzero, counts_nonzero):
        print("Label ", idx, count, np.log10(count))
        multicut_idx = (seg == idx).astype(np.uint8)
        ...
        indices, counts = fast_count_unique(labeled)
        if idx == 2:
            thresh = 100000
        else:
            thresh = min_segment_size

        counts_thresh = counts[1:][counts[1:] > thresh]
        indices_thresh = indices[1:][counts[1:] > thresh]

        if idx == 2:
            print("EXTRA", 2)
            for idx_th, count_th in zip(indices_thresh, counts_thresh):
                output[labeled == idx_th] = 2
        else:
            for idx_th, count_th in zip(indices_thresh, counts_thresh):
                print("New label", cur_idx, count_th, np.log10(count_th))
                out_idx_th = binary_closing((labeled == idx_th).astype(np.uint16))
                output[out_idx_th] = cur_idx
                cur_idx += 1
    ...
    output[~fg_mask] = 0
    write_volume(f_multi, output, out_key)

    # Watershed remaining parts
    output[~fg_mask] = bg_idx
    out_key = "segmentation/segmentation_corrected_watershed"
    boundaries = boundaries.astype(np.float32)
    segmentation = output.astype(np.uint32)
    print("Segmentation type", segmentation.dtype, segmentation.shape)
    print("Boundaries type", boundaries.dtype, boundaries.shape)
    _, max_id = vigra.analysis.watershedsNew(boundaries, seeds=segmentation, out=segmentation, method="RegionGrowing")
    # 
    write_volume(f_multi, segmentation, out_key)
